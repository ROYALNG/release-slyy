...
namespace DahuaNetSDKCS2
{
    public static class NETClient
    {
        ...
        /// <param name="waittime">wait time 等待时间</param>
        /// <returns>failed return false, successful return true 失败返回false 成功返回true</returns>
        public static bool ControlDevice(IntPtr lLoginID, EM_CtrlType type, IntPtr pInBuf, IntPtr pOutBuf, int waittime)
        {
            //IntPtr lLoginID, EM_CtrlType emType, IntPtr pInBuf, IntPtr pOutBuf, int nWaitTime = 1000
            bool result = false;
            result = OriginalSDK.CLIENT_ControlDevice(lLoginID, type, pInBuf, waittime);
            NetGetLastError(result);
            return result;
        }
