...
namespace DHNetSDK
{
    ...
    public sealed class DHClient
    {
        ...
        public static string DHGetLastError()
        {
        ...
        public static bool DHSetAutoReconnect(fHaveReConnect cbHaveReConnect, IntPtr dwUser)
        {
            return CLIENT_SetAutoReconnect(cbHaveReConnect, dwUser);
        }
        /// <summary>
        ...
        public static bool DHQueryDevState(int lLoginID, int nType, IntPtr pBuf, int nBufLen, out int pRetLen, int waittime)
        {
            return CLIENT_QueryDevState(lLoginID, nType, pBuf, nBufLen, out pRetLen, waittime);
        }
        ...
        public static bool DHPTZControl(int lLoginID, int nChannelID, PTZ_CONTROL dwPTZCommand, ushort dwStep, bool dwStop)
        {
            ...
            switch (dwPTZCommand)
            {
                ...
                case PTZ_CONTROL.EXTPTZ_RIGHTDOWN:
                    returnValue = CLIENT_DHPTZControlEx(lLoginID, nChannelID, (ushort)dwPTZCommand, dwStep, dwStep, 0, dwStop);
                    break;
        ...
        /// <param name="dwStop">是否停止</param>
        /// <returns>true:成功;false:失败</returns>
        public static bool DHPTZControl(int lLoginID, int nChannelID, bool blnFastGoto, uint param1, uint param2, uint param3, bool dwStop)
        {
            bool returnValue = false;
            if (blnFastGoto)
            {
        ...
        /// </summary>
        /// <returns>true:成功;false:失败</returns>
        public static int DHFindFile(int lLoginID, int nChannelId, int nRecordFileType, string cardid, NET_TIME time_start, NET_TIME time_end, bool bTime, int waittime)
        {
            int returnValue;
            returnValue = CLIENT_FindFile(lLoginID, nChannelId, nRecordFileType, null, time_start, time_end, bTime, waittime);
            DHThrowLastError(returnValue);
            return returnValue;
        }
        /// <summary>
        ...
        /// </summary>
        /// <returns>true:成功;false:失败</returns>
        public static int DHFindNextFile(int lFindHandle, NET_RECORDFILE_INFO lpFindData)
        {
            int returnValue;
            returnValue = CLIENT_FindNextFile(lFindHandle, lpFindData);
            DHThrowLastError(returnValue);
            return returnValue;
        }
        ...
        /// </summary>
        /// <returns>true:成功;false:失败</returns>
        public static bool DHFindClose(int lFindHandle)
        {
            bool returnValue = false;
            returnValue = CLIENT_FindClose(lFindHandle);
            DHThrowLastError(returnValue);
            return returnValue;
        }
        ...
        public static bool DHPlayBackControl(int lPlayHandle, PLAY_CONTROL pPlayCommand)
        {
            ...
            switch (pPlayCommand)
            {
                ...
                case PLAY_CONTROL.Slow:
                    ...
                    break;
                case PLAY_CONTROL.Normal:
                    returnValue = CLIENT_NormalPlayBack(lPlayHandle);
                    break;
            }
            DHThrowLastError(returnValue);
            return returnValue;
        ...
        private static NET_TIME ToNetTime(DateTime dateTime)
        {
            ...
            result.dwYear = (uint)dateTime.Year;
            result.dwMonth = (uint)dateTime.Month;
            result.dwDay = (uint)dateTime.Day;
            result.dwHour = (uint)dateTime.Hour;
            result.dwMinute = (uint)dateTime.Minute;
            result.dwSecond = (uint)dateTime.Second;
            return result;
        }
        ...
        #region << 原SDK调用 >>
        public const string SDK_DAHUA = @".\Driver\dahua\dhnetsdk.dll";
        /// <summary>
        /// 返回函数执行失败代码
        /// </summary>
        ...
        [DllImport(SDK_DAHUA)]
        private static extern bool CLIENT_SetAutoReconnect(fHaveReConnect cbHaveReConnect, IntPtr dwUser);
        ...
        /// <returns>true:成功;false:失败</returns>
        [DllImport(SDK_DAHUA)]
        private static extern bool CLIENT_DHPTZControlEx(int lLoginID, int nChannelID, ushort dwPTZCommand, uint lParam1, uint lParam2, uint lParam3, bool dwStop);
        ...
        /// <returns>true:成功;false:失败</returns>
        [DllImport(SDK_DAHUA)]
        private static extern bool CLIENT_NormalPlayBack(int lPlayHandle);
        ...
        /// <returns>true:成功;false:失败</returns>
        [DllImport(SDK_DAHUA)]
        private static extern int CLIENT_FindFile(int lLoginID, int nChannelId, int nRecordFileType, string cardid, NET_TIME time_start, NET_TIME time_end, bool bTime, int waittime);
        ...
        /// <returns>true:成功;false:失败</returns>
        [DllImport(SDK_DAHUA)]
        private static extern int CLIENT_FindNextFile(int lFindHandle, NET_RECORDFILE_INFO lpFindData);
        ...
        /// <returns>true:成功;false:失败</returns>
        [DllImport(SDK_DAHUA)]
        private static extern bool CLIENT_FindClose(int lFindHandle);
        ...
        /// </summary>
        [DllImport(SDK_DAHUA)]
        private static extern bool CLIENT_QueryDevState(int lLoginID, int nType, IntPtr pBuf, int nBufLen, out int pRetLen, int waittime);
    ...
    /// 远程外部报警信息
    /// </summary>
    public struct ALARM_REMOTE_ALARM_INFO
    {
    ...
    public struct ALARM_FRONTDISCONNET_INFO
    {
        public uint dwSize;
        public int nChannelID;
        public int nAction;
    ...
    public enum CALLBACK_TYPE
    {
        ...
        // 编码器报警
        DH_ENCODER_ALARM_EX = 0x210A,
        // 紧急报警
        DH_URGENCY_ALARM_EX = 0x210B,
        // 无线报警
        DH_WIRELESS_ALARM_EX = 0x210C,
        // 报警解码器报警
        DH_ALARM_DECODER_ALARM_EX = 0x210E,
        //静态检测报警
        DH_STATIC_ALARM_EX = 0x2122,
        // 报警布撤防状态
        DH_ALARM_ARM_DISARM_STATE = 0x211D,
        DH_ALARM_CAMERA_MOVE = 0x212F,    // 摄像机移位报警事件(对应结构体ALARM_CAMERA_MOVE_INFO)
        DH_ALARM_FRONTDISCONNECT = 0x2132,  // 前端IPC断网报警(对应结构体ALARM_FRONTDISCONNET_INFO)
        DH_ALARM_ALARM_EX_REMOTE = 0x2133,// 远程外部报警(对应结构体 ALARM_REMOTE_ALARM_INFO)
        DH_ALARM_IVS = 0x2140   // 智能报警事件(对应结构体ALARM_IVS_INFO)
    }
    ...
    public enum QUERY_DEV_STATE
    {
        DH_DEVSTATE_COMM_ALARM = 0x0001,		// 查询普通报警状态(包括外部报警，视频丢失，动态检测)
        DH_DEVSTATE_SHELTER_ALARM = 0x0002,		// 查询遮挡报警状态
        DH_DEVSTATE_RECORDING = 0x0003,		// 查询录象状态
        DH_DEVSTATE_DISK = 0x0004,		// 查询硬盘信息
        DH_DEVSTATE_RESOURCE = 0x0005,		// 查询系统资源状态
        DH_DEVSTATE_BITRATE = 0x0006,		// 查询通道码流
        DH_DEVSTATE_CONN = 0x0007,		// 查询设备连接状态
        DH_DEVSTATE_PROTOCAL_VER = 0x0008,		// 查询网络协议版本号，pBuf = int*
        DH_DEVSTATE_TALK_ECTYPE = 0x0009,		// 查询设备支持的语音对讲格式列表，见结构体DHDEV_TALKFORMAT_LIST
        DH_DEVSTATE_SD_CARD = 0x000A,		// 查询SD卡信息(IPC类产品)
        DH_DEVSTATE_BURNING_DEV = 0x000B,		// 查询刻录机信息
        DH_DEVSTATE_BURNING_PROGRESS = 0x000C,		// 查询刻录进度
        DH_DEVSTATE_PLATFORM = 0x000D,		// 查询设备支持的接入平台
        DH_DEVSTATE_CAMERA = 0x000E,		// 查询摄像头属性信息(IPC类产品)，pBuf = DHDEV_CAMERA_INFO *，可以有多个结构体
        DH_DEVSTATE_SOFTWARE = 0x000F,		// 查询设备软件版本信息
        DH_DEVSTATE_LANGUAGE = 0x0010,		// 查询设备支持的语音种类
        DH_DEVSTATE_DSP = 0x0011,		// 查询DSP能力描述(对应结构体DHDEV_DSP_ENCODECAP)
        DH_DEVSTATE_OEM = 0x0012,		// 查询OEM信息
        DH_DEVSTATE_NET = 0x0013,		// 查询网络运行状态信息
        DH_DEVSTATE_TYPE = 0x0014,		// 查询设备类型
        DH_DEVSTATE_SNAP = 0x0015,		// 查询功能属性(IPC类产品)
        DH_DEVSTATE_RECORD_TIME = 0x0016,		// 查询最早录像时间和最近录像时间
        DH_DEVSTATE_NET_RSSI = 0x0017,      // 查询无线网络信号强度，见结构体DHDEV_WIRELESS_RSS_INFO
        DH_DEVSTATE_BURNING_ATTACH = 0x0018,		// 查询附件刻录选项
        DH_DEVSTATE_BACKUP_DEV = 0x0019,		// 查询备份设备列表
        DH_DEVSTATE_BACKUP_DEV_INFO = 0x001a,		// 查询备份设备详细信息
        DH_DEVSTATE_BACKUP_FEEDBACK = 0x001b,		// 备份进度反馈
        DH_DEVSTATE_ATM_QUERY_TRADE = 0x001c,		// 查询ATM交易类型
        DH_DEVSTATE_SIP = 0x001d,		// 查询sip状态
        DH_DEVSTATE_VICHILE_STATE = 0x001e,		// 查询车载wifi状态
        DH_DEVSTATE_TEST_EMAIL = 0x001f,      // 查询邮件配置是否成功
        DH_DEVSTATE_SMART_HARD_DISK = 0x0020,      // 查询硬盘smart信息
        DH_DEVSTATE_TEST_SNAPPICTURE = 0x0021,     // 查询抓图设置是否成功
        DH_DEVSTATE_STATIC_ALARM = 0x0022,     // 查询静态报警状态
        DH_DEVSTATE_SUBMODULE_INFO = 0x0023,      // 查询设备子模块信息
        DH_DEVSTATE_DISKDAMAGE = 0x0024,      // 查询硬盘坏道能力 
        DH_DEVSTATE_IPC = 0x0025,      // 查询设备支持的IPC能力, 见结构体DH_DEV_IPC_INFO
        DH_DEVSTATE_ALARM_ARM_DISARM = 0x0026,     // 查询报警布撤防状态
        DH_DEVSTATE_ACC_POWEROFF_ALARM = 0x0027,   // 查询ACC断电报警状态(返回一个DWORD, 1表示断电，0表示通电)
        DH_DEVSTATE_TEST_FTP_SERVER = 0x0028,   // 测试FTP服务器连接
        DH_DEVSTATE_3GFLOW_EXCEED = 0x0029,      // 查询3G流量超出阈值状态,(见结构体DHDEV_3GFLOW_EXCEED_STATE_INFO)
        DH_DEVSTATE_3GFLOW_INFO = 0x002a,      // 查询3G网络流量信息,见结构体DH_DEV_3GFLOW_INFO
        DH_DEVSTATE_VIHICLE_INFO_UPLOAD = 0x002b, // 车载自定义信息上传(见结构体ALARM_VEHICLE_INFO_UPLOAD)
        DH_DEVSTATE_SPEED_LIMIT = 0x002c,      // 查询限速报警状态(见结构体ALARM_SPEED_LIMIT)
        DH_DEVSTATE_DSP_EX = 0x002d,      // 查询DSP扩展能力描述(对应结构体DHDEV_DSP_ENCODECAP_EX)
        DH_DEVSTATE_3GMODULE_INFO = 0x002e,     // 查询3G模块信息(对应结构体DH_DEV_3GMODULE_INFO)
        DH_DEVSTATE_MULTI_DDNS = 0x002f,      // 查询多DDNS状态信息(对应结构体DH_DEV_MULTI_DDNS_INFO)
        DH_DEVSTATE_CONFIG_URL = 0x0030,      // 查询设备配置URL信息(对应结构体DH_DEV_URL_INFO)
        DH_DEVSTATE_HARDKEY = 0x0031      // 查询HardKey状态（对应结构体DHDEV_HARDKEY_STATE)
