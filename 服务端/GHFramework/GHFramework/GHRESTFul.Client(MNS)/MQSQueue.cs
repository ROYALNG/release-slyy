...
namespace GHRESTFul.Client
{
    public class MQSQueue
    {
        ...
        public MessageVisibilityChangeResponse changeVisibility(string receiptHandle, int visibilityTimeOut)
        {
            ...
            return this.client.execute<MessageVisibilityChangeResponse>(MQSClient.Method.PUT, string.Format("{0}/{1}", this.name, "messages"), new Dictionary<string, string>(), new MessageVisibilityChangeRequest() { ReceiptHandle = receiptHandle, VisibilityTimeOut = visibilityTimeOut });
        }
